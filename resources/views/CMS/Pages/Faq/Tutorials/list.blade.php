...
@section('header')
    <title>{{sitename()}} - {{tr('Tutorials')}} </title>
    <style>
        .error {
            color: #ff095f;
        }
        ...
        <div class="row">
            <div class="col-xl-12">
                ...
                <div id="panel-1" class="panel">
                    ...
                    <div class="panel-hdr">
                        <h2>
                            {{tr('Tutorials')}} <span class="fw-300"><i>{{tr('topics')}}</i></span>
                        </h2>
                    ...
                    <div class="panel-container show">
                        <div class="panel-content">
                            <div class="col-sm-12 d-flex align-items-center justify-content-end">
                                <a href="{{route('CMS.tutorials.create',$tag->id)}}" class="btn btn-primary waves-effect waves-themed ">
                                    {{tr('add new')}}</a>
                            </div>
                            <br/>
    ...
    <script>
        $(document).ready(function () {
            ...
            var table =  $('#data_table').DataTable({
                ...
                ajax: {
                    url: "{{route('CMS.tutorials.list',$tag->id)}}",
                    type: 'GET',
                },
                columns: [
                    {data: 'ord', name: 'ord'},
            ...
            table.on('row-reorder', function (e, details) {
                if(details.length) {
                    ...
                    $.ajax({
                        ...
                        method: 'POST',
                            url: "{{route('CMS.tutorials.reOrder',$tag->id)}}",
                        data: { rows }
            ...
            $('body').on('click', '.delete', function (e) {
                ...
                Swal.fire(
                    ...
                    if (result.value) {
                        $.post('{{route('CMS.tutorials.delete',$tag->id)}}', {
                            id: id,
                            _token: '{{csrf_token()}}'
                        }).done(function (data) {
