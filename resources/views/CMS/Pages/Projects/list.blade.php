...
@section('header')
    <title>{{sitename()}} - {{tr('projects')}} </title>
    <style>
        .error {
            color: #ff095f;
        }
        ...
        <div class="row">
            <div class="col-xl-12">
                ...
                <div id="panel-1" class="panel">
                    ...
                    <div class="panel-hdr">
                        <h2>
                            {{tr('projects')}} <span class="fw-300"><i>{{tr('list')}}</i></span>
                        </h2>
                        <div class="panel-toolbar">
                            <button class="btn btn-panel" data-action="panel-collapse" data-toggle="tooltip"
                                    data-offset="0,10" data-original-title="{{tr('collapse')}}"></button>
                    ...
                    <div class="panel-container show">
                        <div class="panel-content">
                            <div class="col-sm-12 d-flex align-items-center justify-content-end">
                                <a href="{{route('CMS.projects.create')}}" class="btn btn-primary waves-effect waves-themed ">
                                    {{tr('add new')}}</a>
                            </div>
                            <br/>
        ...
        $(document).ready(function () {
            ...
            table.on('row-reorder', function (e, details) {
                if(details.length) {
                    ...
                    $.ajax({
                        ...
                        method: 'POST',
                            url: "{{route('CMS.projects.reOrder')}}",
                        data: { rows }
            ...
            $('body').on('click', '.delete', function (e) {
                ...
                Swal.fire(
                    ...
                    if (result.value) {
                        $.post('{{route('CMS.projects.delete')}}', {
                            id: id,
                            _token: '{{csrf_token()}}'
                        }).done(function (data) {
