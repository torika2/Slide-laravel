...
@section('header')
    <title>{{sitename()}} - {{tr('software pricing parameters')}} </title>
    <style>
        .error {
            color: #ff095f;
        }
        ...
        <div class="row">
            <div class="col-xl-12">
                <div id="panel-1" class="panel">
                    ...
                    <div class="panel-hdr">
                        <h2>
                            {{tr('software pricing parameters')}} <span class="fw-300"><i>{{tr('list')}}</i></span>
                        </h2>
                    ...
                    <div class="panel-container show">
                        <div class="panel-content">
                            <div class="col-sm-12 d-flex align-items-center justify-content-end">
                                <a href="{{route('CMS.software.pricing.parameters.create')}}" class="btn btn-primary waves-effect waves-themed ">
                                    {{tr('add new')}}</a>
                            </div>
                            <br/>
    ...
    <script>
        $(document).ready(function () {
            ...
            var table =  $('#data_table').DataTable({
                ...
                ajax: {
                    url: "{{route('CMS.software.pricing.parameters.list')}}",
                    type: 'GET',
                },
                columns: [
                    {data: 'ord', name: 'ord'},
            ...
            table.on('row-reorder', function (e, details) {
                if(details.length) {
                    ...
                    $.ajax({
                        ...
                        method: 'POST',
                            url: "{{route('CMS.software.pricing.parameters.reOrder')}}",
                        data: { rows }
            ...
            $('body').on('click', '.delete', function (e) {
                ...
                Swal.fire(
                    ...
                    if (result.value) {
                        $.post('{{route('CMS.software.pricing.parameters.delete')}}', {
                            id: id,
                            _token: '{{csrf_token()}}'
                        }).done(function (data) {
