...
@section('header')
    <title>{{sitename()}} - {{tr('partners')}} </title>
    <style>
        .error {
            color: #ff095f;
        }
        ...
        <div class="row">
            <div class="col-xl-12">
                            ...
                            <form method="post" id="form" action="{{route('CMS.static.update')}}" enctype="multipart/form-data">
                                ...
                                <div class="form-row">
                                    <div class="form-group col-md-6">
                                        ...
                                        <a href="{{route('CMS.partners.list')}}"
                ...
                <div id="panel-1" class="panel">
                    ...
                    <div class="panel-hdr">
                        <h2>
                            {{tr('partners')}} <span class="fw-300"><i>{{tr('list')}}</i></span>
                        </h2>
                        <div class="panel-toolbar">
                            <button class="btn btn-panel" data-action="panel-collapse" data-toggle="tooltip"
                                    data-offset="0,10" data-original-title="{{tr('collapse')}}"></button>
                    ...
                    <div class="panel-container show">
                        <div class="panel-content">
                            <div class="col-sm-12 d-flex align-items-center justify-content-end">
                                <a href="{{route('CMS.partners.create')}}" class="btn btn-primary waves-effect waves-themed ">
                                    {{tr('add new')}}</a>
                            </div>
                            <br/>
                            <table class="table table-bordered table-hover table-striped w-100" id="partners_table">
                                <thead>
                                <tr>
                                    <th>ord</th>
                                    <th>{{tr('name')}}</th>
...
@section('scripts')
    ...
    <script>
        $(document).ready(function () {
            ...
            table.on('row-reorder', function (e, details) {
                if(details.length) {
                    ...
                    $.ajax({
                        ...
                        method: 'POST',
                            url: "{{route('CMS.partners.reOrder')}}",
                        data: { rows }
            ...
            $('body').on('click', '.delete-partners', function (e) {
                e.preventDefault();
                ...
                Swal.fire(
                    {
                        title: "{{tr('are you sure, you want to delete partner?')}}",
                        type: "question",
                    ...
                    if (result.value) {
                        $.post('{{route('CMS.partners.delete')}}', {
                            id: id,
                            _token: '{{csrf_token()}}'
                        }).done(function (data) {
                            if (data.success === true) {
                                Swal.fire('{{tr('partner removed successfully!')}}', '', "success");
                                $('#partners_table').DataTable().ajax.reload();
                            }
                        });
