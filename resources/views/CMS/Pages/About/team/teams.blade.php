        ...
        <div class="row">
            <div class="col-xl-12">
                <div id="panel-1" class="panel">
                    ...
                    <div class="panel-hdr">
                        <h2>
                            {{tr('teams')}} <span class="fw-300"><i>{{tr('list')}}</i></span>
                        </h2>
                    ...
                    <div class="panel-container show">
                        <div class="panel-content">
                            <div class="col-sm-12 d-flex align-items-center justify-content-end">
                                <a href="{{route('CMS.teams.create')}}" class="btn btn-primary waves-effect waves-themed ">
                                    {{tr('add new')}}</a>
                            </div>
                            <br/>
                            <table class="table table-bordered table-hover table-striped w-100" id="teams_table">
                                <thead>
                                <tr>
                                    <th>ord</th>
                                    <th>{{tr('name')}}</th>
    ...
    <script>
        $(document).ready(function () {
            ...
            var table =  $('#teams_table').DataTable({
                ...
                ajax: {
                    url: "{{route('CMS.teams.list')}}",
                    type: 'GET',
                },
                columns: [
                    {data: 'ord', name: 'ord'},
                    {data: 'name.{{App::getLocale()}}', name: 'name'},
                    {data: 'position.{{App::getLocale()}}', name: 'position'},
            ...
            $('body').on('click', '.delete-team', function (e) {
                ...
                Swal.fire(
                    {
                        title: "{{tr('are you sure, you want to delete team?')}}",
                        type: "question",
                    ...
                    if (result.value) {
                        $.post('{{route('CMS.teams.delete')}}', {
                            id: id,
                            _token: '{{csrf_token()}}'
                        }).done(function (data) {
                            if (data.success === true) {
                                Swal.fire('{{tr('team removed successfully!')}}', '', "success");
