...
@section('header')
    <title>{{sitename()}} - {{tr('clients')}} </title>
    <style>
        .error {
            color: #ff095f;
        }
        ...
        <div class="row">
            <div class="col-xl-12">
                <div id="panel-1" class="panel">
                    ...
                    <div class="panel-hdr">
                        <h2>
                            {{tr('clients')}} <span class="fw-300"><i>{{tr('list')}}</i></span>
                        </h2>
                    ...
                    <div class="panel-container show">
                        <div class="panel-content">
                            <div class="col-sm-12 d-flex align-items-center justify-content-end">
                                <a href="{{route('CMS.clients.create')}}" class="btn btn-primary waves-effect waves-themed ">
                                    {{tr('add new')}}</a>
                            </div>
                            <br/>
    ...
    <script>
        $(document).ready(function () {
            ...
            $('body').on('click', '.delete-data', function (e) {
                ...
                Swal.fire(
                    {
                        title: "{{tr('are you sure, you want to delete client?')}}",
                        type: "question",
                    ...
                    if (result.value) {
                        $.post('{{route('CMS.clients.delete')}}', {
                            id: id,
                            _token: '{{csrf_token()}}'
                        }).done(function (data) {
                            if (data.success === true) {
                                Swal.fire('{{tr('client removed successfully!')}}', '', "success");
